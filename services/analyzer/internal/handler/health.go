...
func (h *Handler) checkRabbitMQConsumer() ComponentStatus {
	// We need to add IsHealthy method to rabbitmq.Consumer
	// For now, we assume it's healthy if it exists
...
func (h *Handler) checkRabbitMQPublisher() ComponentStatus {
	// We need to add IsHealthy method to rabbitmq.Publisher
	// For now, we assume it's healthy if it exists
	return ComponentStatus{
		Status: "ok",
		Details: map[string]interface{}{
...
func (h *Handler) checkMinIO(ctx context.Context) ComponentStatus {
	...
	return ComponentStatus{
		Status: "ok",
		Details: map[string]interface{}{
			"bucket": h.cfg.MinIO.OriginalBucket,
		},
	}
}
